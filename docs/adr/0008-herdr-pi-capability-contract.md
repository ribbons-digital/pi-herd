...
Slice 10 applies the Pi extension contract through one `/herd` slash command whose handler receives string args and `ctx.cwd`, parses raw send-message text deliberately, invokes the existing CLI with inherited Herdr and Pi env, and avoids agent-callable tools or destructive actions.
