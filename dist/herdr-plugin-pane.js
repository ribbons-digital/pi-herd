...
// src/run-state.ts
import { access as access2, lstat as lstat2, mkdir, readFile as readFile2, readdir, realpath, rename, rm, stat, writeFile as writeFile2 } from "node:fs/promises";
import { constants as constants2 } from "node:fs";
import { basename, dirname as dirname2, isAbsolute as isAbsolute2, join, relative as relative2, resolve as resolve3, sep as sep2 } from "node:path";
...
function roleWorktreePath(repoRoot, runId, role) {
  return resolve2(repoRoot, DEFAULT_WORKTREES_DIR, "pi-herd", runId, role);
}
...
async function createWorktreeHerdrFirst(options) {
  ...
  if (git2.exitCode !== 0) {
    ...
    const gitDetail = firstLine(git2.stderr) || firstLine(git2.stdout) || git2.error?.message || "git worktree add failed";
...
async function inferCanonicalRootFromGitCommonDir(cwd, runner) {
  ...
  const commonDir = resolve3(cwd, result.stdout.trim());
...
function resolveRunsRoot(repoRoot, runsDir) {
  ...
  const runsRoot = resolve3(repoRoot, runsDir);
...
async function allocateRunDirectory(repoRoot, runsRoot, timestamp, baseSlug) {
  ...
  for (let index = 1; index < 1e3; index += 1) {
    ...
    const runDir2 = join(runsRoot, runId2);
    try {
      await mkdir(runDir2);
...
async function writeJsonAtomic(path, value) {
  const tempPath = join(dirname2(path), `.tmp-${process.pid}-${Date.now()}-${randomUUID()}.json`);
  await writeFile2(tempPath, `${JSON.stringify(value, null, 2)}
`, { encoding: "utf8", flag: "wx" });
  await rename(tempPath, path);
}
async function updateRunState(path, mutate) {
  const lockDir = join(dirname2(path), ".state.lock");
...
async function acquireStateLock(lockDir) {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    try {
      await mkdir(lockDir);
      try {
        await writeFile2(join(lockDir, "owner.json"), JSON.stringify(owner), "utf8");
      } catch (error) {
...
async function readStateLockOwner(lockDir) {
  try {
    return JSON.parse(await readFile2(join(lockDir, "owner.json"), "utf8"));
  } catch {
    return null;
  }
}
...
// src/status.ts
import { mkdir as mkdir2, readFile as readFile3, rename as rename2, stat as stat2, writeFile as writeFile3 } from "node:fs/promises";
...
async function assertExpectedRoleWorktree(runner, worktreePath, branch, expectedPath, role, repoRoot) {
  if (resolve4(worktreePath) !== resolve4(expectedPath)) {
  ...
  if (resolve4(root.stdout.trim()) !== resolve4(worktreePath)) {
...
async function collectRun(options) {
  ...
  const finalSummaryPath = join2(persisted.state.canonical_run_dir, "FINAL_SUMMARY.md");
...
async function artifactStatuses(state, record) {
  ...
  for (const name of record.required_artifacts) {
    const path = join2(state.canonical_run_dir, name);
    const status = { role: record.role, name, path, present: false, valid: false, stale: false, bytes: 0 };
    try {
      const [raw, fileStat] = await Promise.all([readFile3(path), stat2(path)]);
      status.present = true;
...
async function collectPaneLogs(state, runner) {
  ...
  for (const record of roleEntries(state)) {
    ...
    const logsDir = join2(state.canonical_run_dir, "logs");
    await mkdir2(logsDir, { recursive: true });
    const path = join2(logsDir, `${record.role}-${safeFilename(record.herdr_pane_id)}.log`);
...
async function writeTextAtomic(path, value) {
  const tempPath = join2(dirname3(path), `.tmp-${process.pid}-${Date.now()}-${randomUUID3()}-${basename2(path)}`);
  await writeFile3(tempPath, value, { encoding: "utf8", flag: "wx" });
...
// src/herdr-plugin-action.ts
import { basename as basename4, delimiter, dirname as dirname5, isAbsolute as isAbsolute3, resolve as resolve6 } from "node:path";
...
async function checkGitRepo(options) {
  ...
  if (result.exitCode === 0) {
    return pass("git.repo", "Git repository", firstLine2(result.stdout) || "repository detected");
  }
...
async function checkCommandPresent(options, command, label) {
  ...
  if (result.exitCode === 0) {
    return pass(`command.${command}`, label, firstLine2(result.stdout) || `${command} found`);
  }
...
async function checkHerdrIntegration(options) {
  ...
  if (result.exitCode === 0) {
    return pass("herdr.integration", "Herdr Pi integration", firstLine2(result.stdout) || "integration status succeeded");
  }
  if (result.error?.code === "ENOENT") {
...
// src/init.ts
import { access as access5, mkdir as mkdir3, readFile as readFile4, writeFile as writeFile4 } from "node:fs/promises";
...
async function updateGitignore(cwd, result) {
  const path = resolve5(cwd, ".gitignore");
  let existing = "";
  if (await exists2(path)) {
...
async function assertCurrentPaneIsNotActiveLead(options, runner) {
  ...
  for (const run of runs) {
    let state;
    try {
      state = await readRunState(join5(run.canonical_run_dir, "state.json"));
    } catch {
      continue;
    }
    if (state.status !== "active") {
...
async function leadStatus(options) {
  ...
  for (const role of roleEntries2(state)) {
...
async function leadBrief(options) {
  ...
  const warnings = roleEntries2(state).flatMap(capabilityWarnings);
  const lines = [
    `# pi-herd brief`,
    `Run: ${state.run_id}`,
    `Goal: ${state.goal}`,
    ...
    "",
    "## Roles",
    ...roleEntries2(state).map((role) => `- ${role.role}: ${role.status}; pane=${role.herdr_pane_id ?? "none"}; worktree=${role.worktree_status}`),
    "",
    "## Artifacts",
...
async function artifactInventory(state) {
  ...
  for (const role of roleEntries2(state)) {
    for (const name of role.required_artifacts) {
      const path = join6(state.canonical_run_dir, name);
      artifacts.push({ role: role.role, name, path, present: await exists3(path) });
    }
  }
  return artifacts;
}
async function inboxInventory(state) {
  try {
    ...
    return (await readdir2(join6(state.canonical_run_dir, "inbox"))).sort().slice(0, 20);
  } catch {
    return [];
  }
}
...
// src/herdr-plugin-action.ts
var PLUGIN_ACTIONS = ["doctor", "start", "status", "collect", "cleanup"];
var RUN_SELECTOR_OPTIONS = /* @__PURE__ */ new Set(["--run", "--config"]);
var DOCTOR_VALUE_OPTIONS = /* @__PURE__ */ new Set(["--config"]);
var DOCTOR_BOOLEAN_OPTIONS = /* @__PURE__ */ new Set(["--json"]);
var CLEANUP_DESTRUCTIVE_OPTIONS = /* @__PURE__ */ new Set(["--complete", "--abandon", "--close-panes", "--remove-worktrees", "--force"]);
var PANE_LOOKUP_TIMEOUT_MS = 5e3;
async function runHerdrPluginAction(options) {
  const env = options.env ?? process.env;
  const pluginRoot = resolve6(options.pluginRoot ?? env.HERDR_PLUGIN_ROOT ?? process.cwd());
  const runner = options.runner ?? nodeCommandRunner;
  const main2 = options.main ?? main;
...
`);
  ...
  try {
    ...
    return await main2(cliArgs, targetCwd);
  } catch (error) {
    process.stderr.write(`${error instanceof Error ? error.message : String(error)}
`);
    return 1;
...
function parsePluginAction(value) {
  if (!value) return null;
  return PLUGIN_ACTIONS.includes(value) ? value : null;
}
function parsePluginEntrypointAction(value) {
  if (value === "start-help") return value;
  return parsePluginAction(value);
}
function buildPluginCliArgs(action, args) {
  if (action === "doctor") {
...
function parseRunSelectorArgs(args) {
  return parseSupportedArgs(args, RUN_SELECTOR_OPTIONS, /* @__PURE__ */ new Set(), "--run and --config");
}
function parseDoctorArgs(args) {
  return parseSupportedArgs(args, DOCTOR_VALUE_OPTIONS, DOCTOR_BOOLEAN_OPTIONS, "--config and --json");
}
function parseSupportedArgs(args, valueOptions, booleanOptions, supportedDescription) {
  const parsed = [];
  for (let index = 0; index < args.length; index += 1) {
    const arg = args[index];
...
async function resolvePluginTargetCwd(options) {
...
function cwdFromPluginContext(contextJson, basePath = process.cwd()) {
...
async function cwdFromHerdrPane(options, contextPaneId) {
...
function paneIdFromPluginContext(contextJson) {
  if (!contextJson) return null;
  try {
...
function firstPath(basePath, ...values) {
  for (const value of values) {
    if (typeof value !== "string") continue;
    const trimmed = value.trim();
    if (!trimmed) continue;
    return isAbsolute3(trimmed) ? trimmed : resolve6(basePath, trimmed);
  }
  return null;
}
function applyHerdrBinPath(env) {
  if (!env.HERDR_BIN_PATH || !isAbsolute3(env.HERDR_BIN_PATH)) return;
  const herdrDir = dirname5(env.HERDR_BIN_PATH);
  const currentPath = process.env.PATH ?? "";
  const parts = currentPath.split(delimiter).filter(Boolean);
  if (parts.includes(herdrDir)) return;
  process.env.PATH = [herdrDir, currentPath].filter(Boolean).join(delimiter);
}
function startUsageText() {
  return "Herdr 0.7.1 plugin actions do not pass goal text to actions. Run `pi-herd start <goal>` from the project checkout.\n";
}
function isDirectPluginActionEntrypoint() {
  const entrypoint = process.argv[1];
  return Boolean(entrypoint && ["herdr-plugin-action.ts", "herdr-plugin-action.js"].includes(basename4(entrypoint)) && import.meta.url === pathToFileURL2(entrypoint).href);
}
...
// src/herdr-plugin-pane.ts
var PLUGIN_PANES = ["run-board", "start-wizard", "send-message"];
function formatRefreshInterval(ms) {
  return ms > 0 && Number.isInteger(ms) && ms % 1e3 === 0 ? `${ms / 1e3}s` : `${ms}ms`;
}
async function runHerdrPluginPane(options) {
  const env = options.env ?? process.env;
...
`);
  ...
  const autoRefreshIntervalMs = options.autoRefreshIntervalMs ?? 1e4;
  output.write(`
...
`);
  ...
  const queueRender = (manual = false) => {
    if (renderInFlight) {
...
async function renderBoardSafely(options, output) {
  try {
...
function parsePluginPane(value) {
  if (!value) return null;
  return PLUGIN_PANES.includes(value) ? value : null;
}
async function renderBoardOnce(options, output) {
...
async function runStartWizardSafely(options, output) {
  const readline = options.createReadline?.() ?? createInterface({ input: defaultStdin, output: defaultStdout });
  try {
...
async function runSendMessageSafely(options, output) {
  const readline = options.createReadline?.() ?? createInterface({ input: defaultStdin, output: defaultStdout });
  try {
    const { env, runner, targetCwd } = await resolvePaneRuntime(options);
    ...
    const result = await sendMessage({ cwd: targetCwd, role, message, run: runAnswer || void 0, runner, env });
    output.write(result.text);
    return 0;
  } catch (error) {
    output.write(`${error instanceof Error ? error.message : String(error)}
...
async function resolvePaneRuntime(options) {
  const env = options.env ?? process.env;
...
function isDirectPluginPaneEntrypoint() {
  const entrypoint = process.argv[1];
  return Boolean(entrypoint && ["herdr-plugin-pane.ts", "herdr-plugin-pane.js"].includes(basename5(entrypoint)) && import.meta.url === pathToFileURL3(entrypoint).href);
}
